...
public class StudentDirectoryTest {
	...
	public void testLoadStudentsFromFile() {
		...
		try {
		    sd.loadStudentsFromFile("test-files/i_do_not_exist.txt");
		    fail();
		...
		//Test invalid file
		sd.loadStudentsFromFile("test-files/invalid_student_records.txt");
		assertEquals(0, sd.getStudentDirectory().length);
	...
	public void testAddStudent() {
		...
		try {
		    sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, 25);	
		    fail();
		} catch(IllegalArgumentException e) {
			assertEquals(1, studentDirectory.length);			
		}
		...
		// double password
		try {
		    sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "millkshakes", 25);	
		    fail();
		} catch(IllegalArgumentException e) {
		...
		//same name
		sd.addStudent("BOB", LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
	...
	public void testGetStudentById() {
		...
		// Checks that student object can be retrieved by id correctly
		Student s1 = sd.getStudentById("zking");
		assertEquals(s1.getId(), "zking");
